...
![Version](https://img.shields.io/badge/version-0.2.6-8b5cf6)
![Platforms](https://img.shields.io/badge/Windows_·_Linux_·_macOS-portable-2ea44f)
![Nodes](https://img.shields.io/badge/内置节点-170-8b5cf6)
...
> [!NOTE]
> **✨ 0.2.6 新特性** —— AI 生成进化为 **Agent**（逐步搭图 · 运行观察 · 自适应）· 端口 **AI 预测**（悬停即荐即连）· 一键 **整理节点**（按视口自适应打包）· 3 个「出题」节点（LSB 嵌入 / ZIP 创建 / ZIP 伪加密）· **Windows / Linux / macOS** 三平台绿色版。
...
- 🧩 **170 个内置节点 · 13 大类** —— 从 Base 家族、经典/现代密码、哈希，到隐写、图像频域分析、压缩包解包与「出题」节点，开箱即用（[分类见下](#节点分类)）。
...
| 隐写术 | 8 | 零宽字符（含自动识别）· StegCloak · 空白/SNOW · LSB 图像隐写（提取 + **嵌入出题**） |
| 输入输出 | 5 | 文本输入/输出 · 文件导入 · 图片输入 · 文件输出 |
| 字符编码 | 4 | GBK/UTF 等字符集互转 |
| 压缩包 | 4 | ZIP/7z/TAR/GZIP/RAR 解包 · **ZIP 创建** · **ZIP 伪加密（出题）** |
...
│   ├── src/nodes/           # 170 个内置节点实现（一文件一节点或一对编解码）
